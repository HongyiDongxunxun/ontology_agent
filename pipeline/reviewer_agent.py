...
REVIEWER_PROMPT = (
    ...
    "7. **技术类实体**: 当前本体无 Technology。5G/AI/大数据/VR/AR/云计算等技术体系不得误归为 technique、instrument 或 algorithm；按语境归为 concept、standard、information_system 或 software\n"
    "8. **时间/地名过滤**: 单纯年份不是 stage；单纯城市/地名不是 governance，除非明确指向发展阶段或治理机构\n"
    "9. **无效判定审查**: 检查是否误将图情领域专业术语、具体文献名、具体系统名误判为无效实体\n\n"
    "## Workflow\n"
    ...
    "- 5G/5G技术/VR/AR/人工智能技术/大数据/云计算 → Abstract.Conceptual.concept (作为技术体系/应用概念时)\n"
    "- 第五代移动通信系统/5G通信制式 → Artifact.Normative.standard (强调技术标准/通信制式时)\n"
    "- 具体数字图书馆平台/智慧图书馆系统 → Artifact.System.information_system\n"
    "- 电子计算机/机器人/智能设备 → 普通设备种类通常无效；作为技术概念讨论时可为 concept；只有测量/采集用途才是 instrument\n"
    "- 2005年/20世纪90年代 → 无效时间表达；只有明确时段性划分才是 Event.Event.stage\n"
    "- 深圳/青岛/北京 → 无效地名；只有具体政府机构才是 Agent.Organization.governance\n"
    ...
    "### 例5 — 不认同: technique 兜底错误 (2分)\n"
    "原句: 5G技术为智慧图书馆提供高速低时延连接。\n"
    "分类结果: l1=Abstract, l2=Methodological, l3=technique\n"
    "审查: likert_score=2, comment=5G是技术体系概念,不是研究操作手段\n"
    "suggested_correction=将l2改为Conceptual,l3改为concept\n\n"
    "### 例6 — 不认同: instrument 误用 (2分)\n"
    "原句: 电子计算机推动了图书馆自动化的发展。\n"
    "分类结果: l1=Artifact, l2=Tool, l3=instrument\n"
    "审查: likert_score=2, comment=电子计算机不是测量工具,不应归为instrument\n"
    "suggested_correction=若作为技术概念讨论,改为Abstract.Conceptual.concept\n\n"
    "### 例7 — 不认同: 时间/地名误判 (2分)\n"
    "原句: 2005年深圳档案信息化建设进展较快。\n"
    "分类结果: 2005年→stage; 深圳→governance\n"
    "审查: likert_score=2, comment=年份和城市名本身不是阶段或治理机构\n"
    "suggested_correction=将二者判为无效实体\n\n"
    "### 例8 — 不认同 (2分)\n"
